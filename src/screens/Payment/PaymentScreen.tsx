...
export default function PaymentScreen() {
  ...
  return <SafeAreaView style={s.page}>
    ...
    {loading ? <ActivityIndicator size="large" color="#00C878" style={{ flex: 1 }} /> : <ScrollView contentContainerStyle={s.content}>
      <Text style={s.hint}>เลือกวิธีชำระเงินก่อนเริ่มชาร์จ</Text>
      <TouchableOpacity style={[s.option, method === 'CREDIT' && s.selected]} onPress={() => setMethod('CREDIT')}><Ionicons name="wallet-outline" size={25} color="#00A86B" /><View style={s.optionText}><Text style={s.name}>Wallet</Text><Text style={s.sub}>ยอดคงเหลือ {balance?.toFixed(2) ?? '-'} บาท</Text></View>{method === 'CREDIT' && <Ionicons name="checkmark-circle" size={24} color="#00C878" />}</TouchableOpacity>
      <TouchableOpacity style={[s.option, method === 'CREDIT_CARD' && s.selected]} onPress={() => setMethod('CREDIT_CARD')}><Ionicons name="card-outline" size={25} color="#00A86B" /><View style={s.optionText}><Text style={s.name}>Credit Card</Text><Text style={s.sub}>กันวงเงินก่อนเริ่มชาร์จ</Text></View>{method === 'CREDIT_CARD' && <Ionicons name="checkmark-circle" size={24} color="#00C878" />}</TouchableOpacity>
      {method === 'CREDIT_CARD' && cards.map(card => <TouchableOpacity key={card.id} style={[s.card, cardId === card.id && s.selected]} onPress={() => setCardId(card.id)}><Text style={s.name}>{card.brand ?? 'Card'} •••• {card.lastDigits ?? ''}</Text>{cardId === card.id && <Ionicons name="checkmark" size={20} color="#00C878" />}</TouchableOpacity>)}
...
const s = StyleSheet.create({ page:{flex:1,backgroundColor:'#F8FAFC'},header:{padding:20,flexDirection:'row',justifyContent:'space-between',alignItems:'center'},title:{fontSize:18,fontWeight:'800',color:'#111827'},content:{padding:20},hint:{color:'#64748B',marginBottom:18},option:{backgroundColor:'#FFF',borderRadius:16,padding:18,flexDirection:'row',alignItems:'center',marginBottom:12,borderWidth:1,borderColor:'#E2E8F0'},selected:{borderColor:'#00C878',borderWidth:2},optionText:{flex:1,marginLeft:14},name:{color:'#111827',fontWeight:'800',fontSize:16},sub:{color:'#64748B',marginTop:3},card:{backgroundColor:'#FFF',marginLeft:12,marginBottom:8,borderRadius:12,padding:16,flexDirection:'row',justifyContent:'space-between',borderWidth:1,borderColor:'#E2E8F0'},warning:{color:'#B45309',marginTop:8},button:{backgroundColor:'#00C878',margin:20,height:54,borderRadius:16,justifyContent:'center',alignItems:'center'},disabled:{opacity:.6},buttonText:{color:'#FFF',fontWeight:'800',fontSize:16} });
